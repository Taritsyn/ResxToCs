...
namespace ResxToCs.DotNet
{
	class Program
	{
		...
		static int Main(string[] args)
		{
			...
			Console.WriteLine(@"Processing of resx-files in ""{0}"" directory:", currentDirPath);

			foreach (string filePath in Directory.EnumerateFiles(currentDirPath, "*.resx", SearchOption.AllDirectories))
			{
				GenerateResx(filePath);

				string relativeFilePath = filePath.Substring(currentDirPath.Length);
				Console.WriteLine(@"	* ""{0}"" is processed", relativeFilePath);
			}
		...
		private static void GenerateResx(string filePath)
		{
